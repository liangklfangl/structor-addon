...
function initServer() {
  if (serverRef) {
    ...
    serverRef.app.use(
      rewrite("/structor-deskpage/*", "/structor-desk/index.html")
    );
    serverRef.app.use("/structor-desk", express.static(config.deskDirPath()));
    serverRef.app.use(
      "/structor-gengine-scaffolds",
      express.static(config.scaffoldsDirPath())
    );
  }
}
...
export function setProxyURL(options) {
  return config.projectProxyURL(options.proxyURL).then(() => {
    initProxyServer();
  });
}
...
export function getComponentNotes(options) {
  return storage.readComponentDocument(
...
export function pregenerate(options) {
  ...
  return storage.getComponentTree().then(index => {
...
export function getScaffoldGenerators(options) {
  ...
  return storage.getScaffoldGenerators().then(generators => {
    if (generators && generators.length > 0) {
      generators.forEach(generator => {
        generator.screenshotFilePath = generator.screenshotFilePath.replace(
          rootDir,
          scaffoldsUrlPrefix
        );
      });
