    ...
    if (text && text.length > 0) {
      ...
      textParts.forEach((p, partIndex) => {
        if (p && p.length > 0) {
          // 填写了组件名称
          if (
            caretPosition > textPartLengthCumulative &&
            textPartLengthCumulative + p.length >= caretPosition
          ) {
            checkPart = text.substr(
              textPartLengthCumulative,
              caretPosition - textPartLengthCumulative
            );
            textPart = p;
      ...
      if (checkPart && checkPart.length > 0) {
        caretPixelPosition = getCaretPixelPosition(
          this.$textProbe,
          text.substr(0, caretPosition)
        );
        const checkText = checkPart.toUpperCase();
        values = this.props.componentNames.filter(
          i => i.toUpperCase().indexOf(checkText) >= 0
        );
        ...
        <div className="form-group" style={formGroupStyle}>
          ...
          <div className="panel panel-default" style={{ marginTop: "1em" }}>
            <div className="panel-body">
              ...
              <p>
                Or you may type the construction of nested components:<strong>component1.component2.componentN</strong>
              </p>
              <p>Press enter to submit.</p>
            </div>
          </div>
          {showPopup && (
            <div className="popover bottom" style={popupStylePositioned}>
              <div className="arrow" />
                ...
                >
                  {popupValues.map((item, index) => {
                    if (selectedItemIndex >= 0 && index === selectedItemIndex) {
                      ...
                      return (
                        <p key={item + index} style={listItemStyle}>
                          <a
                            href="#"
                            data-index={index}
                            onClick={this.handleClickListItem}
